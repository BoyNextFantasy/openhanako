...
Beginner mode tone: encouraging, concrete, and guided. Say that the user can describe the feature in plain language, Satori will help turn it into a plugin plan and scaffold, and Satori can answer questions at any step. Ask:
...
5. Pick the target location:
   ...
   - User-installed plugin: the directory reported by `/api/plugins/settings` or `${SATORI_HOME}/plugins`.
...
- Dev authority is not a manifest permission. Hana grants it from the remembered dev install slot under `${SATORI_HOME}/plugins-dev/`, and Agent dev tools are hidden until the user enables the dev tools setting.
