  ...
  return (
    <div className={`${styles['settings-tab-content']} ${styles['active']}`} data-tab="about">
      ...
      <div className={styles['about-hero']}>
        <img className={styles['about-icon']} src={appIconUrl} alt="Satori" />
        <div className={styles['about-name']}>Satori</div>
        <div className={styles['about-tagline']}>{t('settings.about.tagline')}</div>
        {version && <div className={styles['about-version']}>v{version}</div>}
        <AutoUpdateStatus
          state={autoUpdate}
          agentName={settingsConfig?.agent?.name || 'Muse'}
      ...
      <SettingsSection>
        ...
        <SettingsRow
          ...
          control={<span>© 2026 Satori Team</span>}
        />
        <SettingsRow
          label="GitHub"
          control={
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                hana?.openExternal?.('https://github.com/BoyNextFantasy');
              }}
            >
              github.com/BoyNextFantasy
