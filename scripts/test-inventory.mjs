...
function analyzeFile(file) {
  ...
  const flags = {
    ...
    hasFsTmp: /\bmkdtemp(?:Sync)?\b|\btmpdir\b|\bSATORI_HOME\b|\.hanako(?:-dev)?/.test(haystack),
    ...
    hasPersistence: /migration|persist|store|registry|sidecar|sqlite|jsonl|yaml|atomic|write|read|cache|SATORI_HOME|mkdtemp/i.test(haystack),
...
function deleteOrMergeCandidate(flags, category, decision) {
  ...
  if (flags.hardcodesHanaHome) candidates.push("replace hardcoded Hana data directory with injected temp SATORI_HOME");
