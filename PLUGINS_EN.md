...
2. Open Satori ‚Ü?Settings ‚Ü?Plugins, drag the folder into the install area (or drag a .zip)
...
- **Drag-and-drop**: Drag a plugin folder or .zip into Settings ‚Ü?Plugins install area
- **File picker**: Click the install area and select a plugin folder or .zip via the file picker
- **Manual**: Place the plugin directory in `${SATORI_HOME}/plugins/`. The actual path is shown in Settings ‚Ü?Plugins or via `/api/plugins/settings` as `plugins_dir`
...
```text
my-plugin/
‚îú‚îÄ‚îÄ manifest.json          # Optional, only needed for complex declarations
‚îú‚îÄ‚îÄ tools/                 # Tools (called by Agent)
‚î?  ‚îî‚îÄ‚îÄ *.js
‚îú‚îÄ‚îÄ skills/                # Knowledge injection (Markdown)
‚î?  ‚îî‚îÄ‚îÄ my-skill/
‚î?      ‚îî‚îÄ‚îÄ SKILL.md
‚îú‚îÄ‚îÄ commands/              # User commands (slash-triggered)
‚î?  ‚îî‚îÄ‚îÄ *.js
‚îú‚îÄ‚îÄ agents/                # Agent templates (JSON)
‚î?  ‚îî‚îÄ‚îÄ *.json
‚îú‚îÄ‚îÄ routes/                # HTTP routes (requires full-access)
‚î?  ‚îî‚îÄ‚îÄ *.js
‚îú‚îÄ‚îÄ providers/             # Provider declarations: chat/media capabilities (requires full-access)
‚î?  ‚îî‚îÄ‚îÄ *.js
‚îú‚îÄ‚îÄ extensions/            # Pi SDK extension factories (requires full-access)
‚î?  ‚îî‚îÄ‚îÄ *.js
‚îî‚îÄ‚îÄ index.js               # Optional, stateful plugin entry point, loaded last (requires full-access)
```
...
`minAppVersion` (optional) declares the minimum Satori version required to run the plugin. If the current app version is lower, the plugin will not load and its status is set to `incompatible`. All plugins should declare this field to prevent compatibility issues on older versions.

The user must enable the "Allow full-access plugins" toggle in Settings ‚Ü?Plugins. **When the toggle is off, full-access plugins are not loaded at all** (no partial loading) until the user explicitly enables it.
...
### Routes (HTTP Routes) ‚ö?full-access
...
app.post("/create-session", async (c) => {
  ...
  // reqCtx.principal        identity behind this request (owner device / this plugin's iframe surface‚Ä?, null when invoked directly in tests
...
### Providers (Provider Contribution) ‚ö?full-access
...
### Page (Plugin Page) ‚ö?full-access
...
- A plugin can declare both a `page` and a `widget` simultaneously ‚Ä?they are independent
...
### Widget (Sidebar Component) ‚ö?full-access

A plugin can register a component in the right-side Jian sidebar. A widget and a page can be declared simultaneously in the same plugin ‚Ä?they are independent and do not conflict.

```json
{
  "contributes": {
    "widget": {
      "title": { "zh": "ÁõØÁõò", "en": "Monitor" },
...
### SettingsTab (Native Settings Page, Built-ins Only) ‚ö?full-access
...
```json
{
  ...
  "contributes": {
    "settingsTab": {
      "id": "mcp",
      "title": { "zh": "ËøûÊé•Âô?, "en": "Connectors" },
...
## Stateful Plugins (Lifecycle) ‚ö?full-access
...
export default class MyPlugin {
  async onload() {
    ...
    // this.ctx.bus          ‚Ä?EventBus (full: emit/subscribe/request/handle)
    // this.ctx.config       ‚Ä?Config read/write (get/set)
    // this.ctx.dataDir      ‚Ä?Private data directory path
    // this.ctx.log          ‚Ä?Logger with pluginId prefix
    // this.ctx.pluginId     ‚Ä?Plugin ID
    // this.ctx.pluginDir    ‚Ä?Plugin installation directory
    // this.ctx.registerTool ‚Ä?Dynamic tool registration (returns cleanup function)
...
**Error handling**:
- No handler ‚Ü?throws `BusNoHandlerError`
- Timeout (default 30s) ‚Ü?throws `BusTimeoutError`
- Handler business errors ‚Ü?propagated directly
...
### Dynamic Tool Registration ‚ö?full-access
...
### Background Tasks ‚ö?full-access

Plugins can register background tasks so Satori can track and abort them. Runtime lifecycle is managed by `TaskRegistry`.
