/**
 * media-utils.js �?Bridge 媒体工具�?
 *
 * 对标 OpenClaw �?loadWebMedia + splitMediaFromOutput�?
 * 集中处理入站媒体下载和出站回复媒体提取�?
 */
...
/**
 * 设置允许读取的本地目录白名单�?
 * �?BridgeManager 初始化时调用，传�?SATORI_HOME �?workspace�?
 */
...
export async function downloadMedia(url) {
  // protocol-relative URL 补全（QQ �?CDN 可能返回 //domain/path�?
  ...
  if (url.startsWith("http://") || url.startsWith("https://")) {
    ...
    // 流式收集 chunks，避�?arrayBuffer() 的额外一次完整拷�?
    const chunks = [];
  ...
  if (path.isAbsolute(localPath)) {
    // 解析 symlink 再校验白名单，防�?symlink 绕过
...
/** file:// URI �?本地路径（跨平台：Windows �?new URL().pathname 会多一个前�?/�?*/
 ...
 *
 * 安全规则�?
 * - 不从 fenced code block 内提�?
 * - 无效 URL 和本地路径静默丢弃；本地文件必须通过 stage_files 归属�?SessionFile
 *
 * @param {string} text
