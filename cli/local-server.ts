...
export function resolveCliHanaHome(env = process.env) {
  return resolveHomePath(env.SATORI_HOME || path.join(os.homedir(), ".hanako"));
}
...
export function readLocalServerInfo({ hanaHome = resolveCliHanaHome(), checkProcess = true } = {}) {
  ...
  if (!fs.existsSync(filePath)) {
    return {
      ...
      filePath,
      message: `No running Satori Server was found at ${filePath}`,
    };
  }
  ...
  if (checkProcess && Number.isInteger(info.pid) && !isProcessAlive(info.pid)) {
    return {
      ...
      filePath,
      message: `Satori Server process ${info.pid} is no longer running`,
    };
  }
